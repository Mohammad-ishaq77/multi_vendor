...
const Checkout = () => {
  ...
  const handlePlaceOrder = async (e) => {
    ...
    if (paymentMethod === "online" && !paymentDetails.card && !paymentDetails.upi) {
      return alert("Enter a card number or UPI ID to continue.");
    }
    ...
    const order = {
      ...
      total,
      status: "Placed",
      paymentStatus: paymentMethod === "online" ? "Paid (demo gateway)" : "Pending",
            ...
            >
              ...
              <div className="space-y-3">
                {[
                  ...
                  { id: "online", label: "Online Payment", desc: "Pay securely via UPI / Card.", icon: CreditCard },
                ].map((method) => {
                  ...
                  return (
                    <motion.label
                      ...
                      className={`flex items-center gap-4 border rounded-xl p-4 cursor-pointer transition-all ${
                        isActive
                          ? "border-[#155c43] bg-[#f0f8f3] shadow-md shadow-[#155c43]/5"
                          : "border-gray-200 hover:border-gray-300 bg-white"
                      }`}
                    >
                      <div
                        className={`w-5 h-5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                          isActive ? "border-[#155c43]" : "border-gray-300"
                        }`}
                      >
                        <AnimatePresence>
                          {isActive && (
                      ...
                      <div>
                        <p className="font-semibold text-sm text-[#14261f]">{method.label}</p>
