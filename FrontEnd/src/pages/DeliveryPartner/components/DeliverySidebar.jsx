...
const menuItems = [
  { name: "Dashboard", icon: LayoutDashboard, path: "/deliverypartner/dashboard", accent: "from-emerald-500 to-teal-500", accentBg: "bg-emerald-50", accentText: "text-emerald-600" },
  { name: "Available Deliveries", icon: Package, path: "/deliverypartner/available", accent: "from-blue-500 to-indigo-500", accentBg: "bg-blue-50", accentText: "text-blue-600", badge: true },
  { name: "Active Delivery", icon: Truck, path: "/deliverypartner/active", accent: "from-amber-500 to-orange-500", accentBg: "bg-amber-50", accentText: "text-amber-600" },
  { name: "Delivery History", icon: ClipboardList, path: "/deliverypartner/history", accent: "from-violet-500 to-purple-500", accentBg: "bg-violet-50", accentText: "text-violet-600" },
  { name: "Earnings", icon: IndianRupee, path: "/deliverypartner/earnings", accent: "from-teal-500 to-emerald-500", accentBg: "bg-teal-50", accentText: "text-teal-600" },
  { name: "Notifications", icon: Bell, path: "/deliverypartner/notifications", accent: "from-rose-500 to-pink-500", accentBg: "bg-rose-50", accentText: "text-rose-600" },
  { name: "Profile", icon: User, path: "/deliverypartner/profile", accent: "from-slate-500 to-gray-600", accentBg: "bg-slate-50", accentText: "text-slate-600" },
];
      ...
      >
        ...
        <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto scrollbar-hide">
          {menuItems.map((item, i) => {
            ...
            const isActive = location.pathname === item.path || (item.path !== "/deliverypartner/dashboard" && location.pathname.startsWith(item.path));
                ...
                >
                  <motion.div
                    ...
                    className={`relative flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 ${
                      isActive ? "bg-emerald-50/80" : "hover:bg-gray-50/80"
                  ...
                  >
                      ...
                      {!isCollapsed && (
                        <motion.span
                          ...
                          className={`relative z-10 font-semibold text-[0.82rem] whitespace-nowrap overflow-hidden transition-colors ${
                            isActive ? "text-emerald-700" : isHovered ? "text-gray-900" : "text-gray-500"
                          }`}
                        >
                          {item.name}
                        </motion.span>
                    ...
                    {!isCollapsed && item.badge && availableCount > 0 && (
                      <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="relative z-10 ml-auto bg-blue-500 text-white text-[0.6rem] font-bold px-1.5 py-0.5 rounded-full min-w-[1.25rem] text-center shadow-sm">
                        {availableCount}
                      </motion.span>
                    )}
                    {!isCollapsed && item.name === "Notifications" && unreadCount > 0 && (
                      <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="relative z-10 ml-auto bg-rose-500 text-white text-[0.6rem] font-bold px-1.5 py-0.5 rounded-full min-w-[1.25rem] text-center shadow-sm">
                        {unreadCount}
                      </motion.span>
                    )}
                  </motion.div>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between border-b border-gray-100 p-5">
                ...
                <button type="button" onClick={onMobileClose} className="flex h-9 w-9 items-center justify-center rounded-xl text-gray-400 hover:bg-gray-100 transition-colors" aria-label="Close navigation">
                      ...
                      {({ isActive }) => (
                        <>
                          <div className={`w-9 h-9 rounded-[11px] flex items-center justify-center transition-all ${
                            isActive ? `bg-gradient-to-br ${item.accent} text-white shadow-md` : "bg-gray-50 text-gray-400"
                          }`}>
                            <Icon className="w-[18px] h-[18px]" strokeWidth={isActive ? 2.2 : 2} />
                          </div>
                          <span>{item.name}</span>
                          {item.badge && availableCount > 0 && (
                            <span className="ml-auto rounded-full bg-blue-500 px-1.5 py-0.5 text-[0.6rem] font-bold text-white">{availableCount}</span>
                          )}
                          {item.name === "Notifications" && unreadCount > 0 && (
                            <span className="ml-auto rounded-full bg-rose-500 px-1.5 py-0.5 text-[0.6rem] font-bold text-white">{unreadCount}</span>
                          )}
                        </>
                      )}
                    </NavLink>
          ...
          {menuItems.slice(0, 5).map((item) => {
            ...
            return (
              <NavLink key={item.name} to={item.path} className="relative flex flex-col items-center gap-1 p-2">
                {isActive && (
                  <motion.div layoutId="dpMobileNav" className="absolute -top-2 w-8 h-[3px] bg-gradient-to-r from-emerald-500 to-teal-500 rounded-b-full" transition={{ type: "spring", stiffness: 350, damping: 30 }} />
                )}
                <motion.div animate={{ scale: isActive ? 1.1 : 1 }} className={`w-10 h-10 rounded-[11px] flex items-center justify-center transition-all ${isActive ? `bg-gradient-to-br ${item.accent} text-white shadow-md` : "bg-gray-50 text-gray-400"}`}>
                  <Icon className="w-[18px] h-[18px]" strokeWidth={isActive ? 2.2 : 2} />
                </motion.div>
                <span className={`text-[0.65rem] font-semibold ${isActive ? "text-emerald-600" : "text-gray-400"}`}>{item.name.split(" ")[0]}</span>
                {item.badge && availableCount > 0 && (
                  <span className="absolute top-1 right-1 w-4 h-4 bg-blue-500 text-white text-[0.55rem] font-bold rounded-full flex items-center justify-center shadow-sm">{availableCount}</span>
                )}
                {item.name === "Notifications" && unreadCount > 0 && (
                  <span className="absolute top-1 right-1 w-4 h-4 bg-rose-500 text-white text-[0.55rem] font-bold rounded-full flex items-center justify-center shadow-sm">{unreadCount}</span>
                )}
              </NavLink>
            );
          })}
