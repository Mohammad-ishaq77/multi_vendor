...
const SectionHeader = ({ title, subtitle, actionLabel, onAction }) => (
  <div className="mb-5 flex items-end justify-between sm:mb-6">
    ...
    {actionLabel && (
      <button
        onClick={onAction}
        className="flex-shrink-0 inline-flex items-center gap-1 rounded-full bg-[#155c43]/5 px-3 py-2 text-xs font-semibold text-[#155c43] transition-colors hover:bg-[#155c43]/10 sm:px-4 sm:text-sm"
      >
        {actionLabel}
        <span>→</span>
      </button>
    ...
    <AnimatePresence>
      {visible && (
        <motion.button
          ...
          className="fixed bottom-20 lg:bottom-6 right-5 z-50 flex h-10 w-10 items-center justify-center rounded-full bg-[#155c43] text-white shadow-lg shadow-[#155c43]/25 sm:bottom-6 sm:right-6 sm:h-11 sm:w-11"
...
const HeroBanner = ({ greeting, name, isOnline, toggleAvailability }) => (
  ...
  >
    <div className={`relative p-5 sm:p-8 lg:p-10 min-h-[200px] sm:min-h-[220px] md:min-h-[240px] flex flex-col justify-center text-white ${isOnline ? "bg-gradient-to-br from-[#155c43] via-[#1a6b4e] to-[#0d4a32]" : "bg-gradient-to-br from-gray-700 via-gray-600 to-gray-800"}`}>
      ...
      <div className="relative z-10 flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="max-w-xl">
          <div className="mb-3 flex flex-wrap items-center gap-2">
            <span className={`inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[10px] font-medium tracking-wide uppercase backdrop-blur-sm sm:text-xs ${
          ...
          <p className="mt-3 max-w-md text-sm leading-relaxed sm:text-base opacity-80">
            {isOnline
        ...
        <motion.button
          ...
          className={`relative z-10 inline-flex items-center gap-2 self-start rounded-xl px-5 py-3 text-sm font-semibold shadow-md transition-colors sm:self-center sm:px-6 sm:py-3.5 ${
            isOnline
...
const StatCard = ({ icon: Icon, label, value, accent = "emerald", delay = 0 }) => {
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -4 }}
      className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 hover:border-gray-200/80 transition-all duration-300"
    >
...
const ActiveDeliveryCard = ({ delivery }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300"
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <p className="text-lg font-bold text-[#155c43]">₹{delivery.partnerEarning}</p>
      </div>
      ...
      {action && (
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-[#155c43] hover:bg-[#104b36] shadow-md shadow-[#155c43]/20 transition-all"
        >
          {action.label}
...
const RecentDeliveryRow = ({ delivery }) => (
  <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow">
    <div className="flex items-center gap-3">
      <div className="w-10 h-10 rounded-xl bg-[#155c43]/10 flex items-center justify-center text-[#155c43]">
        <Package className="w-5 h-5" />
      </div>
      <div>
        <p className="text-sm font-semibold text-gray-900">{delivery.id}</p>
        <p className="text-xs text-gray-400">{delivery.shopName} → {delivery.customerName}</p>
      </div>
    </div>
    <div className="text-right">
      <p className="text-sm font-bold text-[#155c43]">₹{delivery.partnerEarning}</p>
...
const IdentityCard = ({ profile }) => (
  <motion.div
    ...
    viewport={{ once: true }}
    transition={{ duration: 0.4, ease: [0.22, 1, 0.36, 1] }}
    className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300 overflow-hidden"
  >
    <div className="bg-gradient-to-r from-[#155c43] to-emerald-600 px-5 py-3">
    ...
    <div className="p-5 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-semibold text-gray-900">{profile.identity?.fullName || profile.name}</span>
      </div>
      <div className="flex items-center justify-between">
        <span className="text-xs text-gray-500">Aadhaar Number</span>
        <span className="text-sm font-semibold text-gray-900 font-mono">{profile.identity?.aadhaar || "XXXX XXXX XXXX"}</span>
      </div>
      <div className="flex items-center justify-between">
        <span className="text-xs text-gray-500">Date of Birth</span>
        <span className="text-sm font-semibold text-gray-900">{profile.identity?.dob ? new Date(profile.identity.dob).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" }) : "—"}</span>
      </div>
      <div className="flex items-center justify-between">
  ...
  >
    <div className="bg-gradient-to-r from-gray-700 to-gray-800 px-5 py-3">
      <div className="flex items-center gap-2 text-white">
        ...
        <span className="text-xs font-semibold uppercase tracking-wider">Vehicle Details</span>
      </div>
    </div>
    <div className="p-5 space-y-3">
      <div className="flex items-center justify-between">
        <span className="text-xs text-gray-500">Vehicle Type</span>
        <span className="text-sm font-semibold text-gray-900">{profile.vehicleType || "—"}</span>
      </div>
      <div className="flex items-center justify-between">
        <span className="text-xs text-gray-500">Registration No.</span>
        <span className="text-sm font-semibold text-gray-900 font-mono">{profile.vehicleNumber || "—"}</span>
      </div>
      <div className="flex items-center justify-between">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm font-semibold text-gray-900 truncate max-w-[160px]">{profile.email || "—"}</span>
      </div>
    </div>
  </motion.div>
);
    ...
    >
      <div className="bg-gradient-to-r from-[#155c43] to-emerald-600 px-5 py-3">
        <div className="flex items-center gap-2 text-white">
          ...
          <span className="text-xs font-semibold uppercase tracking-wider">Delivery Guidelines</span>
        </div>
      </div>
      <div className="p-5 space-y-3">
        {guidelines.map((g, i) => (
          <div key={i} className="flex items-start gap-3">
            ...
            <p className="text-sm text-gray-600 leading-relaxed pt-1">{g.text}</p>
          </div>
        ))}
        <button
          onClick={() => navigate("/delivery/onboarding/guidelines")}
          className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-[#155c43] hover:underline"
        >
  ...
  >
    ...
    <div className="grid grid-cols-2 gap-2">
      <button
        ...
        disabled={!isOnline}
        className="flex flex-col items-center gap-2 p-3 rounded-xl bg-[#155c43]/5 hover:bg-[#155c43]/10 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
      >
        <Package className="w-5 h-5 text-[#155c43]" />
        <span className="text-xs font-semibold text-gray-700">New Deliveries</span>
      </button>
      <button
      ...
      >
        ...
        <span className="text-xs font-semibold text-gray-700">Active Delivery</span>
      </button>
      <button
      ...
      >
        ...
        <span className="text-xs font-semibold text-gray-700">Earnings</span>
      </button>
      <button
...
const PerformanceCard = ({ profile }) => (
  ...
  >
    <div className="flex items-center gap-2 mb-4">
      <TrendingUp className="w-4 h-4 text-[#155c43]" />
      <h3 className="text-sm font-bold text-gray-900">Your Performance</h3>
    </div>
    <div className="space-y-3">
      ...
      <div className="grid grid-cols-2 gap-3 pt-2">
        <div className="text-center p-3 bg-gray-50 rounded-xl">
          <p className="text-lg font-bold text-gray-900">{profile.completedDeliveries || 0}</p>
          <p className="text-[0.65rem] text-gray-500">Completed</p>
        </div>
        <div className="text-center p-3 bg-gray-50 rounded-xl">
          <p className="text-lg font-bold text-gray-900">{profile.cancelledDeliveries || 0}</p>
          <p className="text-[0.65rem] text-gray-500">Cancelled</p>
        </div>
      </div>
    </div>
  </motion.div>
    ...
    <PageTransition>
      <div className="space-y-10 sm:space-y-12">
        ...
        <motion.section initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-5 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300">
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
              <div className="text-center p-3 bg-[#155c43]/5 rounded-xl">
                <p className="text-lg font-bold text-[#155c43]">₹{earnings.today}</p>
                <p className="text-[0.65rem] text-gray-500 mt-0.5">Today</p>
              </div>
              <div className="text-center p-3 bg-emerald-50 rounded-xl">
                <p className="text-lg font-bold text-emerald-600">₹{earnings.thisWeek}</p>
                <p className="text-[0.65rem] text-gray-500 mt-0.5">This Week</p>
              </div>
              <div className="text-center p-3 bg-blue-50 rounded-xl">
                <p className="text-lg font-bold text-blue-600">₹{earnings.thisMonth}</p>
                <p className="text-[0.65rem] text-gray-500 mt-0.5">This Month</p>
              </div>
              <div className="text-center p-3 bg-amber-50 rounded-xl">
                <p className="text-lg font-bold text-amber-600">₹{earnings.total}</p>
        ...
        {recentDeliveries.length > 0 && (
          <motion.section initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }}>
            ...
            <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true }} className="grid grid-cols-1 md:grid-cols-2 gap-3">
...
const DeliveryCardMini = ({ delivery }) => {
  ...
  return (
    <motion.div whileHover={{ y: -4 }} className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-300">
      ...
      <button
        ...
        className="w-full flex items-center justify-center gap-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-[#155c43] bg-[#155c43]/5 hover:bg-[#155c43]/10 transition-all"
      >
