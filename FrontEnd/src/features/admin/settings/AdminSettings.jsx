    ...
    <PageTransition>
      <div className="w-full space-y-6">
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-5">
          {/* General */}
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 shadow-sm space-y-3 h-full">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div><label className="block text-xs font-semibold text-gray-600 mb-1">Platform Name</label><input value={form.platformName || ""} onChange={(e) => setForm({ ...form, platformName: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:border-[#155c43] outline-none" /></div>
              <div><label className="block text-xs font-semibold text-gray-600 mb-1">Support Email</label><input type="email" value={form.supportEmail || ""} onChange={(e) => setForm({ ...form, supportEmail: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:border-[#155c43] outline-none" /></div>
              <div className="sm:col-span-2"><label className="block text-xs font-semibold text-gray-600 mb-1">Support Phone</label><input value={form.supportPhone || ""} onChange={(e) => setForm({ ...form, supportPhone: e.target.value })} className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:border-[#155c43] outline-none" /></div>
            </div>
          </motion.div>

          {/* Delivery */}
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.04 }} className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 shadow-sm space-y-3 h-full">
            ...
            <div>
              ...
              <input type="number" value={form.maxDeliveryDistance || 20} onChange={(e) => setForm({ ...form, maxDeliveryDistance: Number(e.target.value) })} className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:border-[#155c43] outline-none" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          {/* Revenue */}
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.08 }} className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 shadow-sm space-y-3 h-full">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div><label className="block text-xs font-semibold text-gray-600 mb-1">Delivery Partner %</label><input type="number" value={form.deliveryPartnerPercentage || 80} onChange={(e) => setForm({ ...form, deliveryPartnerPercentage: Number(e.target.value) })} className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:border-[#155c43] outline-none" /></div>
              <div><label className="block text-xs font-semibold text-gray-600 mb-1">NearMart %</label><input type="number" value={form.platformPercentage || 20} onChange={(e) => setForm({ ...form, platformPercentage: Number(e.target.value) })} className="w-full px-3 py-2 rounded-xl border border-gray-200 text-sm focus:border-[#155c43] outline-none" /></div>
            </div>
          ...
          {/* Notifications */}
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.12 }} className="bg-white rounded-2xl border border-gray-100 p-4 sm:p-5 shadow-sm space-y-2.5 h-full">
            ...
            {[
              ...
              <div key={item.key} className="flex items-center justify-between gap-3 p-2.5 bg-gray-50 rounded-xl">
