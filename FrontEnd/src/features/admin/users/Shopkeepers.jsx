...
export default function Shopkeepers() {
  ...
  const { shopkeepers, shops, orders, suspendUser, activateUser } = useAdmin();
  ...
  const enriched = useMemo(() => {
    return shopkeepers.map((sk) => {
      const shop = shops.find((s) => s.ownerId === sk.id);
      const shopOrders = orders.filter((o) => o.shopId === shop?.id);
      return {
        ...sk,
        shopName: shop?.name || sk.shopName,
        totalOrders: shopOrders.length,
        revenue: shopOrders.reduce((sum, o) => sum + o.totalAmount, 0),
        rating: shop?.rating || sk.rating || 0,
      };
    });
  }, [shopkeepers, shops, orders]);

  const filtered = useMemo(() => {
    let result = [...enriched];
    if (search) {
      const q = search.toLowerCase();
      result = result.filter((u) => u.name.toLowerCase().includes(q) || u.email.toLowerCase().includes(q) || (u.shopName && u.shopName.toLowerCase().includes(q)));
    }
    ...
    return result;
  }, [enriched, search, statusFilter, sortOrder]);
  ...
  const confirmAction = async () => {
    ...
    if (actionModal.action === "suspend") suspendUser(actionModal.id, "shopkeeper");
    else activateUser(actionModal.id, "shopkeeper");
    setLoading(false);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage all shopkeeper accounts</p>
        </div>
        <span className="text-sm text-gray-500">{filtered.length} shopkeeper{filtered.length !== 1 ? "s" : ""}</span>
      </div>
      ...
      <div className="bg-white rounded-lg border border-gray-100 shadow-sm">
        <div className="p-4 border-b border-gray-100">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name, email, or shop..."
        ...
        {filtered.length === 0 ? (
          <div className="p-12 text-center">
            ...
            <p className="text-gray-500 font-medium">No shopkeepers found</p>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100">
                    <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-4">Shopkeeper</th>
                    ...
                    <th className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider px-6 py-4">Revenue</th>
                ...
                <tbody>
                  {filtered.map((user, i) => (
                    <motion.tr key={user.id} custom={i} variants={fadeUp} initial="hidden" animate="visible"
                      ...
                      <td className="px-6 py-4 text-sm text-gray-600">{user.shopName}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">{user.totalOrders}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">₹{user.revenue?.toLocaleString()}</td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4">
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[user.status]}`}>{STATUS_LABELS[user.status]}</span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="flex items-center justify-end gap-1">
                          <button onClick={() => navigate(`/admin/users/shopkeeper/${user.id}`)}
                ...
                <motion.div key={user.id} custom={i} variants={fadeUp} initial="hidden" animate="visible" className="p-4">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div><p className="font-medium text-gray-900">{user.name}</p><p className="text-xs text-gray-500">{user.shopName}</p></div>
                    </div>
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${STATUS_COLORS[user.status]}`}>{STATUS_LABELS[user.status]}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center mt-3">
                    <div className="bg-gray-50 rounded-md p-2"><ShoppingBag className="w-3.5 h-3.5 text-gray-400 mx-auto mb-0.5" /><p className="text-xs font-medium text-gray-900">{user.totalOrders}</p><p className="text-[10px] text-gray-500">Orders</p></div>
                    <div className="bg-gray-50 rounded-md p-2"><IndianRupee className="w-3.5 h-3.5 text-gray-400 mx-auto mb-0.5" /><p className="text-xs font-medium text-gray-900">₹{user.revenue?.toLocaleString()}</p><p className="text-[10px] text-gray-500">Revenue</p></div>
                    <div className="bg-gray-50 rounded-md p-2"><Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400 mx-auto mb-0.5" /><p className="text-xs font-medium text-gray-900">{user.rating}</p><p className="text-[10px] text-gray-500">Rating</p></div>
                  </div>
                  <div className="flex items-center gap-1 mt-3">
                    ...
                    {user.status === "active" ? (
                      <button onClick={() => handleAction(user.id, "suspend")}
                        className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-rose-700 bg-rose-50 rounded-lg hover:bg-rose-100 transition-colors"><UserX className="w-3.5 h-3.5" /> Suspend</button>
                    ) : (
                      <button onClick={() => handleAction(user.id, "activate")}
      ...
      <ConfirmModal isOpen={actionModal.open} onClose={() => setActionModal({ open: false, id: null, action: "" })} onConfirm={confirmAction}
        title={actionModal.action === "suspend" ? "Suspend Shopkeeper" : "Activate Shopkeeper"}
        message={actionModal.action === "suspend" ? "This shopkeeper will be suspended. Their shop will be deactivated." : "This shopkeeper will be reactivated."}
        loading={loading} />
    </div>
  );
}
