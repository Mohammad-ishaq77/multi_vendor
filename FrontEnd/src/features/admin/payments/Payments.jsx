...
import { Search, IndianRupee, CheckCircle, Clock, XCircle, RotateCcw } from "lucide-react";
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by order, customer, shop..." className="w-full pl-10 pr-4 py-2.5 rounded-md border border-gray-200 text-sm focus:border-[#155c43] outline-none" />
          </div>
          ...
          <select value={methodFilter} onChange={(e) => setMethodFilter(e.target.value)} className="px-4 py-2.5 rounded-md border border-gray-200 text-sm bg-white">
        ...
        <div className="md:hidden space-y-3">
          {payments.map((p) => (
            <motion.div key={p.orderId} whileHover={{ y: -2 }} onClick={() => navigate(`/admin/payments/${p.orderId}`)} className="bg-white rounded-lg border border-gray-100 p-4 shadow-sm cursor-pointer">
