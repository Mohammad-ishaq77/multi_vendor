...
const navigationGroups = [
  ...
  },
  {
    ...
    items: [
      {
        name: "Shopkeeper Approvals",
        icon: ClipboardCheck,
        path: "/admin/approvals/shopkeepers",
        accent: "from-emerald-500 to-green-500",
        accentBg: "bg-emerald-50",
        accentText: "text-emerald-600",
        badgeKey: "pendingApprovalsCount",
      },
      {
...
const AdminSidebar = ({ isMobileOpen = false, onMobileClose = () => {} }) => {
  ...
  const badgeCounts = useMemo(
    () => ({
      pendingApprovalsCount,
      pendingDeliveryApprovalsCount: pendingApprovalsCount,
      pendingShopTypeRequestsCount,
      unreadNotificationCount,
    }),
    [pendingApprovalsCount, pendingShopTypeRequestsCount, unreadNotificationCount]
  );
      ...
      >
        ...
        <div className={`flex items-center gap-3 px-5 py-5 ${isCollapsed ? "justify-center" : ""}`}>
          ...
          <AnimatePresence>
            {!isCollapsed && (
              ...
              >
                ...
                <p className="text-[0.6rem] text-gray-400 font-medium -mt-0.5">
                  Admin Panel
                </p>
              </motion.div>
            )}
          ...
          <motion.button
            ...
            className={`ml-auto text-gray-400 hover:text-emerald-600 transition-colors rounded-lg hover:bg-emerald-50 p-1 ${
        ...
        <nav className="flex-1 px-3 py-2 space-y-3 overflow-y-auto scrollbar-hide">
          {navigationGroups.map((group) => (
            <div key={group.label}>
              {!isCollapsed && (
                <p className="px-3 mb-1 text-[0.6rem] font-bold tracking-widest text-gray-300 uppercase">
                  {group.label}
                </p>
              )}
